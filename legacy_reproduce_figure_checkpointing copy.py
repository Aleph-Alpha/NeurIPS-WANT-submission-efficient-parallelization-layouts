...
colors = {"Enabled": "blue", "Disabled": "red", "Disabled (No RMSKernel)": "green"}
legend_entries = ["Disabled", "Disabled (No RMSKernel)", "Enabled"]
...
for file_path in file_paths:
    ...
    b2 = plt.bar(
        ...
        color=colors["Disabled (No RMSKernel)"],
        width=bar_width,
    )
    b3 = plt.bar(
    ...
    ANNO_SIZE = 6
    plt.annotate(
        f"({best_entry_disabled['micro_batch_size'].values[0]}, {best_entry_disabled['model_parallel_size'].values[0]}, {best_entry_disabled['pipe_parallel_size'].values[0]})",
    ...
    if file_name == "llama_30B_8k_seq_length_bfloat16_128_GPUs_all_runs":
        plt.text(
            ...
            0.21,  # Adjust the vertical position as needed
            "CUDA Out Of Memory",
            rotation="vertical",
            ha="center",
            va="center",
...
plt.title("Model FLOPs Utilization with Activation Checkpointing Enabled or Disabled", y=1.05)

# Display the plot
plt.tight_layout()
plt.show()
plt.savefig("figs/legacy_fig_checkpointing.pdf")
